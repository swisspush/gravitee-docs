...
NOTE: The `PolicyChain` *must always be called to end an _on Request_ processing*. Be ware to make a call to the `PolicyChain#doNext()`, `PolicyChain#failWith()` or `PolicyChain#doBreak()` to correctly end the _on Request_ processing.
...
NOTE: The `PolicyChain` *must always be called to end an _on Response_ processing*. Be ware to make a call to the `PolicyChain#doNext()`, `PolicyChain#failWith()` or `PolicyChain#doBreak()` to correctly end the _on Response_ processing.
